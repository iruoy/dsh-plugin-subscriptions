...
import { streamWithAuthRetry, mergeReasoning, AccountCatalogCache, discoverAcrossAccounts, discoverOrRetryAuth, isDiscoveryAborted, isMissingOrInvalidCredential, oauthEndpointError, OAuthEndpointError, } from './common.js';
...
export class CopilotAdapter extends LlmAdapter {
    ...
    streamCore(options, account) {
        return streamWithAuthRetry({
            ...
            prepare: async (session, signal) => ({
                ...
                wire: copilotRequestWire(this.configuredWireEntry(options.model) ?? await this.discovered(options.model), options),
            ...
            request: (session, { wire, messages, scope }, signal) => this.request(options, messages, session, signal, wire, scope),
            ...
            parse: (body, pulse, { wire, scope }) => {
                if (wire !== 'responses')
                    return streamChatCompletions(body, pulse);
    ...
    async request(options, messages, session, signal, wire, replayScopeKey) {
