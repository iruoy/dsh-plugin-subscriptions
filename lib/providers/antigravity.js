...
import { httpLlmError, streamWithAuthRetry, mergeReasoning, AccountCatalogCache, discoverOrRetryAuth, isMissingOrInvalidCredential, oauthEndpointError, OAuthEndpointError, discoverAcrossAccounts, isDiscoveryAborted, } from './common.js';
...
import { DEFAULT_RATE_LIMIT_WAIT, DEFAULT_RETRY, durationMs, jsonBody, resetFromFields, resetInstantFromDate, subscriptionRetryPolicy, } from './rate-limit.js';
 ...
 */
const ANTIGRAVITY_RESET_FIELDS = ['quotaResetTimeStamp', 'quotaResetDelay', 'retryDelay'];
 ...
 */
export const antigravityRateLimitReset = (_response, body, now) => {
...
export class AntigravityAdapter extends LlmAdapter {
    ...
    streamCore(options, account) {
        return streamWithAuthRetry({
            ...
            request: (session, messages, signal) => requestAntigravityContent(session, toAntigravityRequest(options, messages, session.projectId, true), true, this.options.runtime, this.options.fetchFn, signal),
    ...
    async generate(options) {
        ...
        if (!response.ok)
            throw await httpLlmError(response, 'Antigravity API', this.errorOptions());
