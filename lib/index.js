    ...
    const speed = {
        async speed(sessionId) {
            ...
            for (const { key } of await codexTokens?.list() ?? []) {
                if (!accountPreferences || !Object.hasOwn(accountPreferences, key) || accountPreferences[key].independentEntry !== true)
                    continue;
                for (const model of [...fastModels]) {
                    if (parseAccountModelId(model))
                        continue;
                    if (await codexAdapter?.supportsFastTier(model, key))
