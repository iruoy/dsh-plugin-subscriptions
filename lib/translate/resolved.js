    ...
    for (const message of messages) {
        ...
        for (const block of message.content) {
            if (block.type !== 'tool-result')
                continue;
            const parts = block.content.filter((part) => part.type === 'image');
            if (parts.length > 0) {
...
export async function resolveImages(messages, attachments, signal) {
    ...
    if (!hasImages(messages))
        return withToolResultBlocks(messages);
    ...
    return withToolResultBlocks(await Promise.all(messages.map(async (message) => ({
        ...message,
